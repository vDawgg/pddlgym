...
def sample_problem(domain, problem_dir, problem_outfile, maze_array):
    ...
    objects.add(player)
    state.add(
        np.random.choice([oriented_down, oriented_left, oriented_right, oriented_up])(
            player
        )
    )
    ...
    for r in range(1, len(maze_array) - 1):
        for c in range(1, len(maze_array[r]) - 1):
            ...
            if maze_array[r - 1][c] != 1:
                state.add(
                    move_dir_up(
                        loc_type(f"loc-{r + 1}-{c + 1}"), loc_type(f"loc-{r}-{c + 1}")
                    )
                )
            if maze_array[r + 1][c] != 1:
                state.add(
                    move_dir_down(
                        loc_type(f"loc-{r + 1}-{c + 1}"),
            ...
            if maze_array[r][c - 1] != 1:
                state.add(
                    move_dir_left(
                        loc_type(f"loc-{r + 1}-{c + 1}"), loc_type(f"loc-{r + 1}-{c}")
                    )
                )
            if maze_array[r][c + 1] != 1:
                state.add(
...
def generate_problems(problem_dir="maze"):
    domain = PDDLDomainParser(
        os.path.join(PDDLDIR, "maze.pddl"),
    ...
    for problem_idx in range(0, 50):
        ...
        maze_size = np.random.randint(
            min_maze_sizes[maze_size_index], max_maze_sizes[maze_size_index]
        )
...
def generate_maze_wilsons(n):
    maze_array = [
        [1 if c == 0 or c == n - 1 or r == 0 or r == n - 1 else 4 for c in range(n)]
        for r in range(n)
    ]  # Borders are blocked (4 is place-holder for Wilson's)
    ...
    while len(valid_additions) > 0 and (valid_additions != old_valid_additions):
        ...
        maze_array, in_maze, new_dead_ends = get_loop_erased_random_walk(
            n, maze_array, in_maze, valid_additions
        )
...
def get_loop_erased_random_walk(n, maze_array, in_maze, valid_additions):
    ...
    assert (
        len(adjacent_to_maze) == 0 or len(adjacent_to_maze) == 1
    )  # Because curr is chosen from valid_additions, should be 1 or 0
    ...
    while len(adjacent_to_maze) != 1:
        valid_neighbors = list(
            (set(get_inbound_neighbors(curr, maze_array)) - visited - dead_ends)
            & valid_additions
        )  # Neighbor should only connect to existing maze at one edge

        if (
            len(valid_neighbors) == 0
        ):  # No valid neighbors, add curr to set of dead ends
            dead_ends.add(curr)
        ...
        if (
            len(adjacent) > 0
        ):  # If cycle (by being in adjacent), remove elements of cycle, and reset curr
    ...
    return (
        ...
        in_maze,
        set(),
    )  # Dead-ends should be empty set since the elements possibly in dead-ends can be used otherwise?
