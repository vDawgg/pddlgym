...
def create_goal(domain, balls, rooms, num_balls):
    ...
    goal_balls = [
        balls[i] for i in np.random.choice(len(balls), replace=False, size=num_balls)
    ]
    goal_lits = []
...
def sample_problem(
    ...
    min_num_rooms=101,
    max_num_rooms=200,
    min_num_balls_goal=5,
    max_num_balls_goal=20,
):

    all_objects, balls, rooms, initial_state = sample_state(
        domain,
...
def generate_problems():
    domain = PDDLDomainParser(
        os.path.join(PDDLDIR, "manygripper.pddl"),
    ...
    for problem_idx in range(40):  # 50):
        ...
        if problem_idx < 40:
            sample_problem(
                ...
                min_num_rooms=10,
                max_num_rooms=20,
                min_num_balls_goal=2,
                max_num_balls_goal=5,
            )
        else:
            sample_problem(
                domain,
