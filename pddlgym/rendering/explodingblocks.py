...
def draw_robot(
    ax, robot_width, robot_height, midx, midy, holding, block_width, block_height
):
...
def draw_blocks(ax, block_width, block_height, block_positions):
    for block_name, ((x, y), destroyed) in block_positions.items():
        ...
        if destroyed:
            mymarker = ax.scatter(
                x + block_width / 2, y + block_height / 2, s=300, c="black", marker="x"
            )
            ax.add_artist(mymarker)
        ax.add_patch(rect)
...
def render(obs, mode="human", close=False):
    ...
    block_width, block_height, block_positions = get_block_params(
        piles, width, height, table_height, robot_height
    )
