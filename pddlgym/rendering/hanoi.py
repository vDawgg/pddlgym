...
def get_objects_from_obs(obs):
    ...
    while len(discs):
        for d1 in discs:
            ...
            raise ValueError(f"Could not find smallest remaining disc in {discs}")
...
def get_disc_params(
    discs_ordered_by_size, peg_to_disc_list, peg_to_hor_midpoints, width, peg_height
):
