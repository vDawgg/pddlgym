...
def sample_problem(
    ...
    blocks, block_state = sample_blocks(
        ...
        pile_heights=np.random.randint(
            1, 3, size=np.random.randint(min_num_piles, max_num_piles + 1)
        ),
    )
    ...
    goal = create_goal(
        ...
        blocks,
        pile_heights=np.random.randint(
            2, 4, size=np.random.randint(min_num_piles_goal, max_num_piles_goal + 1)
        ),
    )
...
def generate_problems():
    domain = PDDLDomainParser(
        os.path.join(PDDLDIR, "manyblockssmallpiles.pddl"),
    ...
    for problem_idx in range(50):
        ...
        if problem_idx < 40:
            ...
            sample_problem(
                ...
                min_num_piles=80,
                max_num_piles=100,
                min_num_piles_goal=5,
                max_num_piles_goal=10,
            )
