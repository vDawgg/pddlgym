...
def sample_state(
    ...
    if randomize_robot_start:
        robot_loc = get_random_location(locations_in_grid, disallowed_mask=wall_mask)
    else:
    ...
    if randomize_hospital_loc:
        hospital_loc = get_random_location(locations_in_grid, disallowed_mask=wall_mask)
    else:
    ...
    for person_idx, person in enumerate(people):
        if randomize_person_loc:
            loc = get_random_location(locations_in_grid, disallowed_mask=wall_mask)
        else:
...
def sample_problem(
    ...
    goal = create_goal(
        domain, people, hospital_loc, num_selected_people=num_selected_people
    )
...
def generate_problems(num_train=50, num_test=10, level=1, **kwargs):
    domain = PDDLDomainParser(
        os.path.join(PDDLDIR, f"{DOMAIN_NAME}.pddl"),
    ...
    while problem_idx < num_train + num_test:
        ...
        problem_id, problem_filepath = sample_problem(
            domain, problem_dir, problem_outfile, **kwargs
        )
