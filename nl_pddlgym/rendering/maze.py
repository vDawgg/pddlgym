...
(
    ...
    GOAL_RIGHT_PLAYER,
    NOT_GOAL_UP_PLAYER,
    NOT_GOAL_LEFT_PLAYER,
    NOT_GOAL_DOWN_PLAYER,
    NOT_GOAL_RIGHT_PLAYER,
    WALL,
    PERSON,
) = range(NUM_OBJECTS)
...
TOKEN_IMAGES = {
    ...
    GOAL_LEFT_PLAYER: np.rot90(
        plt.imread(get_asset_path("sar_robot_holding_person.png")), k=1
    ),
    GOAL_DOWN_PLAYER: np.rot90(
        plt.imread(get_asset_path("sar_robot_holding_person.png")), k=2
    ),
    GOAL_RIGHT_PLAYER: np.rot90(
        plt.imread(get_asset_path("sar_robot_holding_person.png")), k=3
    ),
...
def build_string_grid(obs):
    ...
    grid = [
        ["wall" for _ in range(num_cols)] for _ in range(num_rows)
    ]  # All are walls unless clear or player or goal

    assert player_loc is not None
    assert goal_loc is not None
    ...
    if player_loc == goal_loc:  # If player at goal
        ...
        grid[player_loc[0] - 1][player_loc[1] - 1] = (
            "not_goal_" + orientation + "_player"
        )
