...
def initialize_figure(height, width, fig_scale=1.0, grid_colors=None):
...
def draw_token(token_image, r, c, ax, height, width, token_scale=1.0, fig_scale=1.0):
    oi = OffsetImage(
        token_image, zoom=fig_scale * (token_scale / max(height, width) ** 0.5)
    )
...
def render_from_layout_crisp(layout, get_token_images, tilesize=16):
    ...
    for r in range(height):
        for c in range(width):
            ...
            for im in token_images:
                canvas[
                    r * tilesize : (r + 1) * tilesize, c * tilesize : (c + 1) * tilesize
