...
class PrologInterface:
    ...
    def __init__(
        ...
        self._varnames_to_var = self._create_varname_to_var(
            self._cond_lits, lambda x: self._clean_variable_name(x).lower()
        )
        self._atomname_to_atom = self._create_varname_to_var(
            self._kb, self._clean_atom_name
        )
        ...
        self._constants = (
            constants  # unused now because variables begin with ? by convention
        )
        # print(self._prolog_str)
        # import ipdb; ipdb.set_trace()

    @classmethod
    def _preprocess_negative_literals(cls, kb, conds) -> tuple:
        # Check for negated quantifiers, which we do not handle
        if any(
            (isinstance(c, Exists) or isinstance(c, ForAll)) and c.is_negative
            for c in conds
        ):
        ...
        for p in negated_predicates:
            ...
            if p.name == "=":
                pos_pred = Predicate("neg-eq", p.arity, p.var_types)
            else:
        ...
        for negated_pred, pos_pred in negated_pred_to_pos_pred.items():
            ...
            for objs in get_object_combinations(
                list(objects),
    ...
    def _replace_predicate(cls, conds, from_pred, to_pred):
        ...
        if isinstance(conds, LiteralConjunction):
            return LiteralConjunction(
                cls._replace_predicate(conds.literals, from_pred, to_pred)
            )
        ...
        if isinstance(conds, ForAll):
            assert not conds.is_negative, (
                "Negative universal quantification not implemented (use Exists instead)"
            )
            return ForAll(
                cls._replace_predicate(conds.body, from_pred, to_pred),
                conds.variables,
                is_negative=conds.is_negative,
            )
        if isinstance(conds, Exists):
            assert not conds.is_negative, (
                "Negative exisential quantification not implemented (use ForAll instead)"
            )
            return Exists(
                conds.variables,
                cls._replace_predicate(conds.body, from_pred, to_pred),
    ...
    @staticmethod
    def _get_lits_from_conds(conds) -> list[Literal]:
        if isinstance(conds, list):
            return [
                lit for c in conds for lit in PrologInterface._get_lits_from_conds(c)
            ]
        if isinstance(conds, Literal):
            return [conds]
    ...
    @classmethod
    def _clean_atom_name(cls, atom_name) -> str:
        return atom_name.lower().replace("-", "_")

    @classmethod
    def _clean_variable_name(cls, var_name) -> str:
    ...
    @classmethod
    def _clean_predicate_name(cls, predicate_name) -> str:
        if predicate_name == "=":
            return "predeq"
        return "pred" + predicate_name.lower().replace("-", "_")

    @staticmethod
    def _create_varname_to_var(lits, transformer) -> dict:
        """ """
        vname_to_v = {}
        for lit in lits:
    ...
    def _create_prolog_str(self) -> str:
        """ """
        ...
        goal_str, variables = self._prolog_goal(
            self._conds, self._allow_redundant_variables
        )
        end = self._prolog_end(variables, self._max_assignment_count)
        return "\n".join([preamble, self._kb_str, type_str, goal_str, end])

    @classmethod
    def _prolog_kb_str(cls, kb) -> str:
        """ """
        kb_str = ""
        for lit in sorted(kb, key=lambda lit: lit.predicate.name):
    ...
    @classmethod
    def _prolog_type_str(cls, kb) -> str:
        """ """
    ...
    def _prolog_goal(self, conds, allow_redundant_variables) -> tuple[str, list]:
        """ """
        all_vars = sorted(
            {
                v
                for lit in conds
                for v in self._get_variables(lit, set())
                if v.startswith("?")
            }
        )
        ...
        for v in sorted(all_vars, key=lambda v: v.var_type):
            type_cond_str += "\n\tistype{}({}),".format(
                v.var_type, self._clean_variable_name(v.name)
            )
        if not allow_redundant_variables:
            all_different_str = "\n\tall_different([{}]).".format(
                ",".join(all_vars_cleaned)
            )
        else:
    ...
    def _get_variables(self, lit, free_vars) -> set:
        if isinstance(lit, Literal):
            return {v for v in lit.variables if v not in free_vars}
        if isinstance(lit, (LiteralConjunction, LiteralDisjunction)):
            return {
                v
                for nested_lit in lit.literals
    ...
    def _prolog_goal_line(self, lit) -> str:
        """ """
        if isinstance(lit, LiteralConjunction):
            inner_str = ",".join(self._prolog_goal_line(lit) for lit in lit.literals)
        ...
        if lit.is_negative:
            raise NotImplementedError(
                "Prolog behaves unexpectedly with negative literals"
            )
        ...
        if isinstance(lit, ForAll):
            ...
            assert len(lit.variables) == 1, (
                "TODO: support ForAlls over multiple variables"
            )
            variable = variables
            ...
            pred_str = "foreach(member({}, {}), {})".format(
        ...
        if isinstance(lit, Exists):
            variables = ",".join(
                [
                    self._clean_variable_name(a.name)
    ...
    @classmethod
    def _prolog_preamble(cls, conds) -> str:
...
""".format(pred_definitions)
    ...
    @classmethod
    def _get_predicates_from_literal(cls, lit) -> set:
        if isinstance(lit, Literal):
            return {lit.predicate.positive}
        if isinstance(lit, LiteralConjunction):
            return {
                p for lit in lit.literals for p in cls._get_predicates_from_literal(lit)
            }
    ...
    @classmethod
    def _prolog_end(cls, variables, max_assignment_count) -> str:
        lowercase_vars = ",".join(
            [cls._clean_variable_name(v).lower() for v in variables]
        )
        uppercase_vars = ",".join(
            [cls._clean_variable_name(v).capitalize() for v in variables]
        )
        return """
...
""".format(lowercase_vars, max_assignment_count, uppercase_vars, uppercase_vars)
    ...
    def _parse_output_line(self, output_line) -> list:
        """ """
